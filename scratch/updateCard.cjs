...
const infoGridRegex = /<div className="relative z-10 space-y-2\.5 text-\[11\.5px\] text-slate-800 font-semibold mb-4 pl-0\.5">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<!-- Toll-Free Section -->/m;
// Wait, safer to just replace the Info Grid div
content = content.replace(
...
{/* Social Links Row */}
<div className="relative z-10 flex justify-center items-center gap-4 text-[9px] font-bold text-slate-600 mt-1 pb-2">
