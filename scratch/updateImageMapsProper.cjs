...
const mapping = {
  ...
  donations: '/assets/icons/icon_donations.jpg', // Might need fallback
...
const newImageMap = Object.entries(mapping).map(([k, v]) => `  "${k}": "${v}"`).join(",\n");
...
// We must also update Home.tsx
let homeContent = fs.readFileSync('D:/rp-foundation/src/pages/Home.tsx', 'utf8');
homeContent = homeContent.replace(/const serviceIdToImage: Record<string, string> = \{[\s\S]*?\};/, newDeclaration);
fs.writeFileSync('D:/rp-foundation/src/pages/Home.tsx', homeContent);

console.log('Updated Services.tsx and Home.tsx');
