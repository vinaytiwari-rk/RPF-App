...
router.get("/api/admin/blood_donors", ...admin, async (_req,res)=>{
  try{const result=await pool.query(`SELECT v.id,v.username,v.full_name AS name,v.mobile,v.email,m.blood_group,m.is_active,CONCAT_WS(', ',v.city,v.state) AS location,m.created_at FROM volunteer_blood_memberships m JOIN volunteers v ON v.id=m.volunteer_id ORDER BY m.updated_at DESC`);return res.json({success:true,data:result.rows});}
...
router.get("/api/admin/blood-network/requests", ...admin, async (_req,res)=>{try{const result=await pool.query(`SELECT r.*,v.full_name AS requester_name,v.mobile AS requester_mobile FROM blood_requests r LEFT JOIN volunteers v ON v.id=r.requester_id ORDER BY r.created_at DESC LIMIT 200`);return res.json({success:true,data:result.rows});}catch(error){return res.status(500).json({success:false,error:"Failed to fetch Blood Network requisitions."});}});

router.get("/api/admin/blood-network/summary", ...admin, async (_req,res)=>{try{const [members,groups,requests]=await Promise.all([pool.query(`SELECT COUNT(*)::int AS total,COUNT(*) FILTER(WHERE is_active)::int AS active FROM volunteer_blood_memberships`),pool.query(`SELECT blood_group,COUNT(*)::int AS count FROM volunteer_blood_memberships WHERE is_active=TRUE GROUP BY blood_group ORDER BY blood_group`),pool.query(`SELECT COUNT(*) FILTER(WHERE status='open')::int AS open,COUNT(*) FILTER(WHERE status='cancelled')::int AS cancelled,COUNT(*)::int AS total FROM blood_requests`)]);return res.json({success:true,data:{members:members.rows[0],groups:groups.rows,requests:requests.rows[0]}});}catch(error){return res.status(500).json({success:false,error:"Failed to load Blood Network summary."});}});
...
router.get("/api/admin/system/diagnostics", ...admin, async (_req,res)=>{
  try {
    const required: Record<string,string[]> = {
      ...
      app_settings:["id"], admin_credentials:["id","username","password_hash"], sessions:["id","user_id","token","expires_at"],
      volunteer_blood_memberships:["volunteer_id","blood_group","is_active"], blood_requests:["id","requester_id","blood_group","status","created_at"],
      grievances:["id","status","created_at"], donations:["created_at"], card_applications:["created_at"], health_camps:["date"]
    };
