...
router.get('/api/campaigns', async (_req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT id, "titleEn", "titleHi", "goalAmount", "raisedAmount",
              "imageUrl", "coverImgUrl", urgent, "createdAt", "updatedAt",
              "publishedAt"
         FROM campaigns
...
router.get('/api/admin/campaigns', authenticateToken, requireAdmin, async (_req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, "titleEn", "titleHi", "goalAmount", "raisedAmount",
              "imageUrl", "coverImgUrl", urgent, "createdAt", "updatedAt",
              published, "publishedAt", "publishedBy"
         FROM campaigns
        ORDER BY "createdAt" DESC`
    );
...
// Admin: every new campaign starts as a draft. It can never become public
// merely because someone created a record or supplied a published flag.
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO campaigns
       (id, "titleEn", "titleHi", "goalAmount", "raisedAmount", "imageUrl", "coverImgUrl", urgent, "createdAt", "updatedAt", published)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW(), FALSE)`,
      [
        id,
        String(titleEn).trim(),
        titleHi ? String(titleHi).trim() : null,
        Number(goalAmount) || 0,
...
// Admin: editing any campaign automatically returns it to draft so that
// a changed campaign can never silently remain live without re-approval.
...
// Admin: explicit publication action. This is the only route that can make
// a campaign visible to the public.
router.post('/api/campaigns/:id/publish', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const result = await pool.query(
      `UPDATE campaigns
          SET published = TRUE,
              ...
              "updatedAt" = NOW()
        WHERE id = $2
        RETURNING id, published, "publishedAt"`,
      [null, req.params.id]
    );
    if (!result.rowCount) return res.status(404).json({ error: 'Campaign not found' });
    invalidateCampaignCache();
