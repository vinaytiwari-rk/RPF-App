...
router.put("/api/admin/hq/credentials", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE admin_credentials SET username = $1, password_hash = $2 WHERE id = 'admin' RETURNING username`,
...
router.put("/api/admin/hq/certificates/signatures", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (!service_id || !signatory_1_name || !signatory_1_designation) {
      return res.status(400).json({ success: false, error: "Service, first signatory name and designation are required." });
    }
    const result = await pool.query(`
...
// Canonical service content API. The database stores all localized values in
// one JSONB column named `content`; no language-specific columns are written.
router.put("/api/admin/hq/services/:id/content", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (!content || typeof content !== 'object' || Array.isArray(content)) {
      return res.status(400).json({ success: false, error: "Content must be an object keyed by language code." });
    }
