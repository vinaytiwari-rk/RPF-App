...
router.put("/api/volunteers/:id/approve", authenticateToken, requireAdmin, async (req, res) => { try { const { id } = req.params; const { status } = req.body; await pool.query(`UPDATE volunteers SET approval_status = $1 WHERE id = $2`, [status, id]); res.json({ success: true, message: "Volunteer status updated" }); } catch (error: any) { res.status(500).json({ success: false, error: error.message }); } });
router.put("/api/volunteers/:id/allocate", authenticateToken, requireAdmin, async (req, res) => { try { const { id } = req.params; const { allocation } = req.body; await pool.query(`UPDATE volunteers SET constituency_allocation = $1 WHERE id = $2`, [allocation, id]); res.json({ success: true, message: "Volunteer allocated" }); } catch (error: any) { res.status(500).json({ success: false, error: error.message }); } });
router.post("/api/volunteers/report", authenticateToken, async (req, res) => { try { const { volunteer_id, check_in_time, check_out_time, report_text, location_lat, location_lng } = req.body; await pool.query(`INSERT INTO volunteer_reports (id, volunteer_id, check_in_time, check_out_time, report_text, location_lat, location_lng) VALUES ($1, $2, $3, $4, $5, $6, $7)`, [crypto.randomUUID(), volunteer_id, check_in_time, check_out_time, report_text, location_lat, location_lng]); res.json({ success: true, message: "Report submitted" }); } catch (error: any) { res.status(500).json({ success: false, error: error.message }); } });

router.get("/api/volunteers/me", authenticateToken, async (req: any, res) => {
  try {
...
router.post("/api/volunteers", authenticateToken, async (req: any, res) => { try { const { name, phone, skills } = req.body; const userId = req.user.id; await pool.query(`UPDATE users SET "isVolunteer" = true WHERE id = $1`, [userId]); const volCheck = await pool.query(`SELECT id FROM volunteers WHERE id = $1`, [userId]); if (volCheck.rows.length === 0) { const userRes = await pool.query(`SELECT username, email FROM users WHERE id = $1`, [userId]); const username = userRes.rows[0]?.username || `user_${userId.slice(-6)}`; const email = userRes.rows[0]?.email || null; const regNumber = "RPF-" + new Date().getFullYear() + "-" + Math.floor(1000 + Math.random() * 9000); await pool.query(`INSERT INTO volunteers (id, username, registration_number, full_name, mobile, email, skills, approval_status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [userId, username, regNumber, name || "Citizen", phone || "", email, JSON.stringify(skills ? skills.split(", ") : []), 'approved']); } else { await pool.query(`UPDATE volunteers SET skills = $1, full_name = $2, mobile = $3 WHERE id = $4`, [JSON.stringify(skills ? skills.split(", ") : []), name || "Citizen", phone || "", userId]); } res.json({ success: true }); } catch (error: any) { console.error("Error creating volunteer record:", error); res.status(500).json({ success: false, error: error.message }); } });
