...
function AppContent() {
  ...
  if (!isAuthenticated) {
    return <LoginScreen lang={language} onLoginSuccess={async (role, details) => {
      if (role === "guest") await loginAsGuest();
      else await login({ id: details?.id, role: (details?.role || (role === "volunteer" ? "volunteer" : "citizen")) as any, name: details?.name ?? "Citizen", phone: details?.phone, isVolunteer: role === "volunteer", isDonor: role === "volunteer", janSevaCardStatus: "none" });
    }} />;
  }
...
export default function App() {
  return <AuthProvider><Toaster position="top-right" /><AppProvider><AppContent /></AppProvider></AuthProvider>;
}
