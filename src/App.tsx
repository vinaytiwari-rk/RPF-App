...
function AppContent() {
  ...
  if (!isAuthenticated) return <LoginScreen lang={language} onLoginSuccess={async (role, details) => { if (role === "guest") await loginAsGuest(); else await login({ id: details?.id, role: (details?.role || (role === "admin" ? "admin" : "user")) as any, name: details?.name ?? "User", phone: details?.phone, isVolunteer: details?.role === "volunteer" || role === "volunteer", isDonor: details?.role === "volunteer" || role === "volunteer", janSevaCardStatus: "none" }, details?.token, details?.remember !== false); }} />;
