...
function AppContent() {
  ...
  if (!isAuthenticated) return <LoginScreen lang={language} onLoginSuccess={async (role, details) => { if (role === "guest") await loginAsGuest(); else await login({ id: details?.id, role: (details?.role || (role === "volunteer" ? "volunteer" : "citizen")) as any, name: details?.name ?? "Citizen", phone: details?.phone, isVolunteer: role === "volunteer", isDonor: role === "volunteer", janSevaCardStatus: "none" }, details?.token, details?.remember !== false); }} />;
  return <ErrorBoundary><BrowserRouter><RoutePersistence /><NavigationBridge /><Suspense fallback={<PageLoader />}><Routes><Route element={<MainLayout />}>
    <Route path="/" element={<Home />} /><Route path="/impact" element={<ImpactPage />} /><Route path="/vision-goals" element={<VisionGoalsPage />} /><Route path="/about" element={<VisionGoalsPage />} /><Route path="/sos" element={<SosSystem />} />
