...
export default function LocationPicker({ onLocationSelect, defaultLocation }: LocationPickerProps) {
  ...
  // Auto-trigger geolocation when component mounts
  useEffect(() => {
    if (!defaultLocation) {
      detectLocation();
    }
  ...
  const detectLocation = () => {
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.error(err);
          setErrorMsg('Failed to detect location. Please click on the map.');
          setLoading(false);
        }
      );
    } else {
    ...
    <div className="space-y-3 w-full">
      <div className="flex flex-col sm:flex-row gap-3">
        <input
          type="text"
          readOnly
          className="flex-1 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 block"
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg text-sm px-4 py-2.5 focus:ring-4 focus:ring-blue-300 transition-colors"
        >
      ...
      <div className="h-64 w-full rounded-lg overflow-hidden border border-gray-300 shadow-sm relative z-0">
        <MapContainer 
          center={position || [22.9734, 78.6569]} // Default to India center
          zoom={position ? 15 : 4} 
      ...
      <p className="text-xs text-gray-500">
        ...
        If the GPS is inaccurate, you can click on the map to place the pin manually.
      </p>
    </div>
  );
}
