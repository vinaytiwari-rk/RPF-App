    ...
    const fetchContent = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setContentData(json.success ? json.data : null);
        } else {
          setContentData(null);
        }
      } catch (err) {
    ...
    return (
      <div className="flex-1 flex items-center justify-center min-h-screen bg-slate-50">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs font-bold text-slate-500">Loading service...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-5 flex-1 flex flex-col min-h-screen bg-slate-50/50 pb-24 relative overflow-x-hidden">
      <div className="flex items-center gap-3 border-b border-slate-200/80 pb-4 mb-5 relative z-10">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-display font-extrabold text-base text-[#000080] truncate">
            {locale === "hi" ? serviceMeta.titleHi : serviceMeta.titleEn}
          </h3>
          <p className="text-[10px] text-slate-500 font-bold truncate">
            {locale === "hi" ? serviceMeta.descHi : serviceMeta.descEn}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm mb-5 flex items-center gap-4 relative overflow-hidden">
        ...
        <div>
          ...
          <h4 className="font-bold text-sm text-slate-800 leading-tight">RP Citizen Welfare</h4>
        </div>
      </div>
      ...
      {!contentData ? (
        <div className="bg-white border border-slate-200 rounded-xl p-6 text-center shadow-sm">
          ...
          <p className="text-xs text-slate-500 font-bold">Detailed content for this service is not yet available.</p>
        </div>
      ) : (
                ...
                {resources.map((resource: any, idx: number) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs font-bold text-slate-700 truncate group-hover:text-indigo-700">{resourceTitle}</p>
                    </a>
                  );
                })}
              </div>
