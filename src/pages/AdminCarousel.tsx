...
export default function AdminCarousel() {
  ...
  const move = (index: number, direction: -1 | 1) => setSlides((current) => { const target = index + direction; if (target < 0 || target >= current.length) return current; const next = [...current]; [next[index], next[target]] = [next[target], next[index]]; return next.map((item, i) => ({ ...item, order: i })); });
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const nextSlides = slides.map((s, index) => ({ ...s, order: index }));
      const res = await axios.post("/api/admin/control/cms/publish", { patch: { carouselSlides: nextSlides }, label: "Carousel publish" }, { headers: { Authorization: `Bearer ${token}` } });
      if (res.data?.success === false) throw new Error(res.data?.error || "Save failed");
      const payload = { ...cms, carouselSlides: nextSlides };
      setCms(payload);
      setSlides(nextSlides);
      toast.success("Carousel published with a protected snapshot.");
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-5 pb-28 text-slate-900 sm:px-6"><div className="mx-auto max-w-5xl space-y-5">
    <header className="flex flex-col gap-3 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><button onClick={() => navigate("/admin")} className="mt-0.5 rounded-xl border border-slate-200 p-2 text-slate-600"><ArrowLeft className="h-4 w-4" /></button><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-orange-600">Home Content</p><h1 className="text-xl font-black">Carousel Management</h1><p className="mt-1 text-xs text-slate-500">Upload real RP Foundation posters, edit copy, control order and publish only active slides.</p></div></div><div className="flex gap-2"><button onClick={addSlide} className="inline-flex items-center justify-center gap-2 rounded-xl border border-orange-200 bg-orange-50 px-4 py-2.5 text-xs font-black text-orange-700"><Plus className="h-4 w-4" /> Add Slide</button><button onClick={save} disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#0F3157] px-4 py-2.5 text-xs font-black text-white disabled:opacity-50"><Save className="h-4 w-4" /> {saving ? "Publishing…" : "Save & Publish"}</button></div></header>
