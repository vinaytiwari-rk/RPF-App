...
import { ArrowLeft, RotateCw, Share2, ExternalLink, ChevronLeft, ChevronRight, Globe, Lock } from 'lucide-react';
...
export default function InAppBrowser() {
  ...
  const reload = () => {
    setLoading(true); showControlsTemporarily();
  ...
  return (
    <div className="fixed inset-0 z-[90] flex min-h-[100dvh] flex-col overflow-hidden bg-[#FFF9F0] font-sans">
      <header className={`fixed top-0 inset-x-0 z-40 transition-all duration-300 ${controls || error ? 'translate-y-0 opacity-100' : '-translate-y-20 opacity-0 pointer-events-none'}`}>
        <div className="flex h-16 items-center gap-2 border-b border-slate-200 bg-[#FFF9F0]/98 px-3 shadow-sm backdrop-blur-xl">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2"><Lock className="h-3.5 w-3.5 shrink-0 text-[#167C5A]" /><div className="min-w-0"><p className="truncate text-xs font-bold text-[#14213D]">{pageTitle}</p><p className="truncate text-[10px] text-slate-500">{rawUrl}</p></div></div>
      ...
      <main className="relative flex-1 w-full overflow-hidden bg-white" onDoubleClick={showControlsTemporarily}>
        {!rawUrl ? <div className="flex h-full flex-col items-center justify-center p-6 text-center bg-[#FFF9F0]"><div className="mb-4 flex h-16 w-16 items-center justify-center rounded-3xl bg-white text-[#D97706] border border-amber-100"><Globe className="h-8 w-8" /></div><h2 className="text-base font-bold text-[#14213D]">{pageTitle}</h2><p className="mt-1.5 text-xs text-slate-500">Invalid or unsupported website link.</p><button onClick={() => navigate(-1)} className="mt-6 rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-xs font-semibold text-[#14213D]">Back to Samahit</button></div> : <>
          <iframe ref={frameRef} title="Samahit Views" src={proxyUrl} onLoad={() => { setLoading(false); if (!error) showControlsTemporarily(); }} onError={() => { setLoading(false); setControls(true); }} className="h-full w-full border-0 bg-white" allow="autoplay; clipboard-read; clipboard-write; encrypted-media; fullscreen; geolocation; microphone; camera; picture-in-picture" allowFullScreen />
          {loading && <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center bg-[#FFF9F0]/95 backdrop-blur-sm"><BrandLoader size="lg" label="Opening website…" /><p className="mt-3 text-xs font-semibold text-slate-600">Opening website…</p></div>}
      ...
      <footer className={`fixed bottom-0 inset-x-0 z-40 transition-all duration-300 ${controls || error ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0 pointer-events-none'}`}>
        <div className="flex h-16 items-center justify-around border-t border-slate-200 bg-[#FFF9F0]/98 px-5 shadow-sm backdrop-blur-xl"><button onClick={() => frameHistory('back')} className="flex h-10 w-10 items-center justify-center rounded-xl text-[#14213D] active:scale-95" aria-label="Back"><ChevronLeft className="h-5 w-5" /></button><button onClick={() => frameHistory('forward')} className="flex h-10 w-10 items-center justify-center rounded-xl text-[#14213D] active:scale-95" aria-label="Forward"><ChevronRight className="h-5 w-5" /></button><button onClick={reload} className="flex h-10 w-10 items-center justify-center rounded-xl text-[#14213D] active:scale-95" aria-label="Refresh"><RotateCw className={`h-5 w-5 ${loading ? 'animate-spin' : ''}`} /></button></div>
      </footer>
    </div>
  );
}
