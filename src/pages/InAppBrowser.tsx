...
/**
 * Persistent RPF Web View: website is shown only in the center while the
 * RPF Web View controls and bottom application navigation stay visible.
 */
export default function InAppBrowser() {
  ...
  const frameHistory = (direction: 'back' | 'forward') => {
    try {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex min-h-[100dvh] flex-col bg-white">
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-3 shadow-sm">
        <h1 className="text-[15px] font-black tracking-tight text-[#000080]">RPF Web View</h1>
        <div className="flex items-center gap-1">
          <button onClick={() => frameHistory('back')} aria-label="Move left" className="flex h-10 w-10 items-center justify-center rounded-full text-xl font-bold text-[#000080] active:bg-orange-50">‹</button>
      ...
      <nav className="flex shrink-0 border-t border-slate-200 bg-white px-1 pb-[calc(.35rem+env(safe-area-inset-bottom))] pt-1 shadow-[0_-8px_30px_-20px_rgba(0,0,128,.22)]">
        {[
          ['/', 'Home', '⌂'],
          ['/services', 'Explore', '⌕'],
          ...
          <button key={path} onClick={() => go(path)} className="flex min-h-[68px] flex-1 flex-col items-center justify-center gap-1 rounded-xl text-[10px] font-medium text-slate-500 active:bg-orange-50">
            <span className="text-[20px] leading-none text-[#000080]">{icon}</span>
            <span>{label}</span>
          </button>
        ))}
      </nav>
