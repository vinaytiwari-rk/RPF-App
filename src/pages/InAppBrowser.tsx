...
export default function InAppBrowser(){
 ...
 const[opening,setOpening]=useState(true);const[failed,setFailed]=useState(false);
 const frameUrl=getFrameUrl(current);const useProxy=frameUrl!==current;

 const openWebsite=async()=>{
   setOpening(true);setFailed(false);
   try{
     await Browser.open({url:current,windowName:"_blank"});
     setOpening(false);
   }catch{
     setOpening(false);setFailed(true);
     try{window.open(current,"_blank","noopener,noreferrer");}catch{}
   }
 };

 useEffect(()=>{if(useProxy){setOpening(false);return;}void openWebsite();},[current,useProxy]);

 if(useProxy){
   return <div className="relative flex min-h-screen flex-col bg-white pb-24"><button onClick={()=>navigate(-1)} aria-label="Back" className="absolute left-3 top-3 z-30 flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-white shadow-sm"><ArrowLeft className="h-4 w-4 text-slate-700"/></button><main className="min-h-0 flex-1 bg-white"><iframe src={frameUrl} title="RPF Web Content" className="h-[calc(100vh-96px)] w-full border-0" referrerPolicy="strict-origin-when-cross-origin"/></main></div>;
 }

 return <div className="relative flex min-h-screen items-center justify-center bg-slate-50 px-5 pb-24"><button onClick={()=>navigate(-1)} aria-label="Back" className="absolute left-3 top-3 flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-white shadow-sm"><ArrowLeft className="h-4 w-4 text-slate-700"/></button><section className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-7 text-center shadow-sm"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700"><ShieldCheck className="h-7 w-7"/></div><h1 className="mt-5 text-xl font-black text-slate-900">{opening?"Opening website":"Website opened"}</h1><p className="mt-2 break-all text-xs leading-5 text-slate-500">{current}</p><p className="mt-4 text-sm text-slate-600">This website is opened using the app's real in-app browser, so publisher iframe security restrictions cannot block the page.</p><button type="button" onClick={openWebsite} className="mt-6 inline-flex items-center justify-center gap-2 rounded-2xl bg-[#000080] px-5 py-3 text-sm font-bold text-white shadow-sm"><ExternalLink className="h-4 w-4"/>{failed?"Try Again":"Open Website"}</button></section></div>;
}
