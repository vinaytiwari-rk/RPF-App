...
const toItems = (titles: string[], desc: string): Item[] => titles.map((title) => ({
...
function calculatorUrl(id: string) {
  return `https://www.calculator.net/${special[id] || id}.html`;
}
...
export default function CalculatorCenterPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return groups
      ...
      .map((group) => ({
        ...group,
        items: group.items.filter((item) => !needle || item.title.toLowerCase().includes(needle)),
      }))
  ...
  const openCalculator = (id: string) => {
    if (id === "bmi-calculator") return navigate("/utilities/bmi-calculator");
    if (id === "gst-calculator") return navigate("/utilities/gst-calculator");
    if (id === "split-bill") return navigate("/utilities/split-bill");
    void openExternalLink(calculatorUrl(id), navigate, "Calculator");
  };

  return (
    <Shell title="Calculator Center" icon={<Calculator className="h-4 w-4" />} onBack={() => navigate("/tools")}>
      <div className="space-y-5">
        <section className="rounded-3xl bg-[#07133d] p-5 text-white">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[.2em] text-[#FF9933]">Smart utilities</p>
              <h2 className="mt-2 text-xl font-black">Find the right calculator</h2>
              <p className="mt-1 text-xs leading-5 text-white/65">Search by task or browse a category. External calculators open inside Samahit.</p>
            </div>
            <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-white/10"><Sparkles className="h-5 w-5 text-[#FF9933]" /></div>
          </div>
        ...
        <div className="relative">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search any calculator..." className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 pl-11 text-sm font-medium outline-none transition focus:border-[#07133d] focus:bg-white" />
        </div>

        <div className="flex gap-2 overflow-x-auto pb-1">
          <button onClick={() => setCategory("all")} className={`whitespace-nowrap rounded-full px-4 py-2.5 text-[11px] font-black ${category === "all" ? "bg-[#07133d] text-white" : "border border-slate-200 bg-white text-slate-500"}`}>All</button>
          {groups.map((group) => (
            <button key={group.id} onClick={() => setCategory(group.id)} className={`whitespace-nowrap rounded-full px-4 py-2.5 text-[11px] font-black ${category === group.id ? "bg-[#07133d] text-white" : "border border-slate-200 bg-white text-slate-500"}`}>{group.title}</button>
          ))}
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-6 py-14 text-center">
            ...
            <button onClick={() => { setQuery(""); setCategory("all"); }} className="mt-3 text-xs font-black text-[#07133d]">Clear filters</button>
          </div>
        ) : (
          filtered.map((group) => {
            ...
            return (
              <section key={group.id} className="space-y-2.5">
                <div className="flex items-center gap-2 px-1">
                  <div className="grid h-8 w-8 place-items-center rounded-xl bg-slate-100 text-[#07133d]"><Icon className="h-4 w-4" /></div>
                  <h2 className="font-black text-[#07133d]">{group.title}</h2>
                  <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-black text-slate-400">{group.items.length}</span>
                </div>
                <div className="grid gap-2.5 sm:grid-cols-2">
                  {group.items.map((item) => (
                    <button key={item.id} onClick={() => openCalculator(item.id)} className="group flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition active:scale-[.99]">
                      <div className="min-w-0 flex-1">
                        <div className="text-sm font-black text-[#07133d]">{item.title}</div>
                        <div className="mt-1 text-[11px] leading-4 text-slate-500">{item.desc}</div>
                      </div>
                      <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-slate-50 text-slate-300"><ChevronRight className="h-4 w-4" /></div>
                    </button>
                  ))}
                </div>
              </section>
        ...
        <p className="rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3 text-xs leading-relaxed text-amber-800">Health calculators are for awareness and estimation only. They do not diagnose disease or replace a qualified healthcare professional.</p>
      </div>
    </Shell>
  );
}
