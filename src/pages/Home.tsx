...
useEffect(()=>{if(slide>=slides.length)setSlide(0);},[slide,slides.length]);useEffect(()=>{if(slides.length<2)return;const t=window.setInterval(()=>setSlide(v=>(v+1)%slides.length),5000);return()=>window.clearInterval(t);},[slides.length]);
...
return <main className="min-h-full bg-[#FAFAF7] text-[#12233D]"><div className="mx-auto w-full max-w-3xl px-4 pb-3 pt-3 sm:px-6"><section className="mb-2 overflow-hidden rounded-xl border border-[#E7EAE4] bg-white"><div className="h-9 overflow-hidden"><div className="inline-block min-w-max whitespace-nowrap px-4 text-[10px] font-semibold leading-9 text-[#46566A]" style={{animation:`rpf-pib-marquee ${pibDuration} linear infinite`}}>{pibText}</div></div></section><section className="mb-3 overflow-hidden rounded-xl border border-[#F0DDD5] bg-[#FFFDFC]"><div className="h-9 overflow-hidden"><div className="inline-block min-w-max whitespace-nowrap px-4 text-[10px] font-semibold leading-9 text-[#6B4B47]" style={{animation:`rpf-sachet-marquee ${sachetDuration} linear infinite`}}>{sachetText}</div></div></section><section className="mb-4"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#138808]">RP Foundation</p><h1 className="mt-1 whitespace-nowrap text-[18px] font-black leading-tight tracking-[-.04em] sm:text-[21px]">Namaste, {name} Ji <span className="text-[15px] sm:text-[17px]">🙏</span></h1><div className="mt-1 flex items-center gap-2 text-[10px] font-bold text-[#64748B]"><span>{greeting}</span><span>•</span><MapPin className="h-3.5 w-3.5 text-[#E67817]"/><span className="max-w-[118px] truncate">{locationName}</span>{temperature&&<><span>•</span><CloudSun className="h-3.5 w-3.5 text-[#1D5B93]"/><span>{temperature}</span></>}</div></section><section className="mb-4 rounded-2xl border border-[#E5EAE6] bg-white px-4 py-3"><p className="text-[8px] font-black uppercase tracking-[.16em] text-[#E67817]">Thought of the Day</p><p className="mt-1 text-[12px] font-bold leading-5 text-[#334155]">“Work is worship, and service is the greatest religion.”</p></section><section><div className="mb-2.5 flex items-end justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#E67817]">Discover</p><h2 className="mt-.5 text-[21px] font-black">RP Foundation at Work</h2></div><span className="text-[12px] font-black text-slate-400">{slide+1}/{slides.length}</span></div><motion.article key={`${current?.image||"slide"}-${slide}`} initial={{opacity:.2}} animate={{opacity:1}} className="relative h-[330px] overflow-hidden rounded-[22px] bg-[#0F3157]"><img src={current?.image} alt={current?.titleEn||"RP Foundation initiative"} className="absolute inset-0 h-full w-full object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-[#07182C]/90 via-[#07182C]/20 to-transparent"/><div className="absolute inset-x-0 bottom-0 p-4 text-white"><h3 className="text-[22px] font-black leading-tight">{current?.titleEn}</h3><p className="mt-1 text-[11px] leading-5 text-slate-200">{current?.subEn}</p><button onClick={()=>navigate((current as any)?.route||"/impact")} className="mt-2 inline-flex items-center gap-1 text-[11px] font-black">Explore <ChevronRight className="h-4 w-4"/></button></div></motion.article><div className="mt-3 flex justify-center gap-2">{slides.map((_:any,i:number)=><button key={i} onClick={()=>setSlide(i)} className={`h-2 rounded-full ${slide===i?"w-7 bg-[#FF9933]":"w-2 bg-slate-300"}`}/>)}</div></section><section className="mt-7"><div className="mb-3"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#138808]">Foundation</p><h2 className="mt-.5 text-[21px] font-black">Our Vision & Leadership</h2></div><div className="grid grid-cols-2 gap-3"><button onClick={()=>navigate("/vision-goals")} className="rounded-2xl border border-[#E6EAE6] bg-white p-4 text-left"><Compass className="h-5 w-5 text-[#1D5B93]"/><p className="mt-4 text-[15px] font-black">Our Vision</p><p className="mt-1 text-[10px] text-slate-500">A clear direction for meaningful social impact.</p></button><button onClick={()=>navigate("/founder-message")} className="rounded-2xl border border-[#E6EAE6] bg-white p-4 text-left"><UserRound className="h-5 w-5 text-[#E67817]"/><p className="mt-4 text-[15px] font-black">Founder’s Message</p><p className="mt-1 text-[10px] text-slate-500">A message from Rohit Pandit, Founder of RP Foundation.</p></button></div></section><section className="mt-7"><div className="mb-3"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#E67817]">Quick Access</p><h2 className="mt-.5 text-[21px] font-black">What can we help with?</h2></div><div className="grid grid-cols-2 gap-3">{actions.map(({title,subtitle,icon:Icon,route,accent})=><motion.button key={title} whileTap={{scale:.98}} onClick={()=>navigate(route)} className="min-h-[154px] rounded-2xl border border-[#E6EAE6] bg-white p-4 text-left"><div className={`flex h-10 w-10 items-center justify-center rounded-xl ${accent}`}><Icon className="h-5 w-5"/></div><p className="mt-4 text-[15px] font-black">{title}</p><p className="mt-1 text-[10px] text-slate-500">{subtitle}</p></motion.button>)}</div></section><section className="mt-7 pb-3"><div className="mb-3"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#138808]">Our Impact</p><h2 className="mt-.5 text-[21px] font-black">RP Foundation at Work</h2></div><div className="grid grid-cols-3 overflow-hidden rounded-2xl border border-[#E5EAE6] bg-white">{[{icon:UsersRound,value:"Community",label:"People first"},{icon:Stethoscope,value:"Care",label:"Health initiatives"},{icon:CalendarDays,value:"Active",label:"Foundation work"}].map(({icon:Icon,value,label})=><div key={label} className="border-r border-[#EEF1EE] px-2 py-4 text-center last:border-r-0"><Icon className="mx-auto h-4 w-4 text-[#138808]"/><p className="mt-2 text-[13px] font-black">{value}</p><p className="mt-.5 text-[8px] text-slate-400">{label}</p></div>)}</div></section></div><style>{`@keyframes rpf-pib-marquee{0%{transform:translate3d(0,0,0)}100%{transform:translate3d(-50%,0,0)}}@keyframes rpf-sachet-marquee{0%{transform:translate3d(-50%,0,0)}100%{transform:translate3d(0,0,0)}}`}</style></main>;}
