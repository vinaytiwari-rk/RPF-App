...
export default function MainLayout() {
  ...
  return (
    <div className="min-h-screen w-full overflow-hidden bg-white font-sans selection:bg-orange-100 selection:text-[#000080]" style={tricolorPattern}>
      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-200/80 bg-white/95 px-3.5 pt-safe-header backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-3xl items-center justify-between sm:h-16 sm:px-1">
          <div className="flex min-w-0 items-center gap-2.5">
            {!root ? (
              <motion.button whileTap={{ scale: 0.9 }} whileHover={{ x: -2 }} onClick={() => navigate(-1)} aria-label="Go back" className="flex h-10 w-10 items-center justify-center rounded-full text-[#000080] hover:bg-orange-50">
                <ArrowLeft className="h-5 w-5" />
              </motion.button>
            ) : (
              <motion.div whileHover={{ rotate: -3, scale: 1.04 }} className="relative flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl border border-orange-200 bg-white shadow-sm sm:h-10 sm:w-10">
            ...
            <div className="min-w-0">
              {location.pathname === "/browser" ? (
                ...
                <>
                  <h1 className="truncate bg-gradient-to-r from-[#FF9933] via-[#000080] to-[#138808] bg-clip-text text-[15px] font-black tracking-[-0.02em] text-transparent sm:text-[16px]">
                    समाहित
                  </h1>
                  <p className="truncate text-[9px] font-bold tracking-[.08em] text-[#000080]">RP Foundation</p>
                </>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-0.5">
            {location.pathname === "/browser" ? (
              <motion.button whileTap={{ scale: 0.85, rotate: 180 }} onClick={() => window.dispatchEvent(new CustomEvent("rpf-browser-refresh"))} aria-label="Refresh page" className="flex h-10 w-10 items-center justify-center rounded-full text-[#000080] hover:bg-orange-50">
                <RotateCw className="h-[19px] w-[19px]" />
              </motion.button>
            ) : (
              <>
                <motion.button whileTap={{ scale: 0.88 }} onClick={() => navigate("/tools")} aria-label={language === "hi" ? "टूल्स" : "Tools"} title={language === "hi" ? "टूल्स" : "Tools"} className="flex h-10 w-10 items-center justify-center rounded-full text-[#000080] hover:bg-orange-50">
                  <Wrench className="h-[19px] w-[19px]" />
                </motion.button>
                <motion.button whileTap={{ scale: 0.88, rotate: -12 }} onClick={() => setLanguage(language === "hi" ? "en" : "hi")} className="flex h-10 w-10 items-center justify-center rounded-full text-[#000080] hover:bg-blue-50">
                  <Globe className="h-[19px] w-[19px]" />
                </motion.button>
                <motion.button whileTap={{ scale: 0.88 }} onClick={() => nav("/notifications")} className="relative flex h-10 w-10 items-center justify-center rounded-full text-[#000080] hover:bg-blue-50">
                  <Bell className="h-[19px] w-[19px]" />
                  {unread > 0 && <motion.span animate={{ scale: [1, 1.25, 1] }} transition={{ duration: 1.8, repeat: Infinity }} className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-[#FF9933] ring-2 ring-white" />}
                </motion.button>
                <motion.button whileTap={{ scale: 0.9 }} onClick={() => nav("/profile")} className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full border border-orange-200 bg-orange-50 text-[#000080]">
      ...
      <main className="min-h-0 w-full overflow-x-hidden bg-white/90 pb-[calc(6.5rem+env(safe-area-inset-bottom))]">
      ...
      {guest && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-[#000080]/20 p-4 backdrop-blur-sm sm:items-center">
          <motion.div initial={{ opacity: 0, y: 30, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="w-full max-w-sm rounded-3xl border border-orange-100 bg-white p-6 shadow-2xl">
            <div className="mb-5 flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-orange-100 to-green-100 text-[#000080]">
            ...
            <div className="mt-6 flex gap-2">
              ...
              <button onClick={() => setGuest(false)} className="flex-1 rounded-xl bg-[#000080] px-4 py-3 text-sm font-semibold text-white">Sign in</button>
            </div>
          </motion.div>
        </div>
      )}
