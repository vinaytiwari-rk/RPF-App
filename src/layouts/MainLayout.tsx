  ...
  return (
    <div className="min-h-screen w-full overflow-hidden bg-white font-sans text-slate-900 selection:bg-orange-100 selection:text-[#000080]" style={tricolorPattern}>
      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-200/80 bg-white/95 px-3.5 pt-safe-header backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-3xl items-center justify-between sm:h-16 sm:px-1">
          <div className="flex min-w-0 items-center gap-2.5">
            {!root ? (
              <motion.button whileTap={{ scale: .9 }} whileHover={{ x: -2 }} onClick={() => navigate(-1)} aria-label="Go back" className="flex h-10 w-10 items-center justify-center rounded-full text-[#000080] hover:bg-orange-50">
            ...
            <div className="min-w-0">
              <h1 className="truncate text-[14px] font-black tracking-[-0.02em] text-[#000080] sm:text-[15px]">RPF Seva App</h1>
              <p className="truncate text-[9px] font-bold tracking-[.08em] text-slate-400">SEVA • SAMARPAN • SANKALP</p>
            </div>
          </div>
          <div className="flex items-center gap-0.5">
            <motion.button whileTap={{ scale: .88, rotate: -12 }} onClick={() => setLanguage(language === "hi" ? "en" : "hi")} className="flex h-10 w-10 items-center justify-center rounded-full text-[#000080] hover:bg-blue-50">
              <Globe className="h-[19px] w-[19px]" />
            </motion.button>
            <motion.button whileTap={{ scale: .88 }} onClick={() => nav("/notifications")} className="relative flex h-10 w-10 items-center justify-center rounded-full text-[#000080] hover:bg-blue-50">
            ...
            <motion.button whileTap={{ scale: .9 }} onClick={() => nav("/profile")} className="flex h-9 w-9 items-center justify-center rounded-full border border-orange-200 bg-orange-50 text-[#000080]">
      ...
      {(location.pathname === "/" || location.pathname === "/services") && (
        <motion.button whileTap={{ scale: .88 }} whileHover={{ y: -3, scale: 1.04 }} onClick={() => nav("/donations")} aria-label="Donate" className="fixed bottom-[calc(4.75rem+env(safe-area-inset-bottom))] right-3.5 z-40 flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-[#FF9933] to-[#138808] text-white shadow-lg shadow-green-900/20">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200/80 bg-white/96 px-1 pt-1 pb-[calc(.35rem+env(safe-area-inset-bottom))] shadow-[0_-8px_30px_-20px_rgba(0,0,128,.22)] backdrop-blur-xl">
        <div className="mx-auto flex max-w-3xl items-center justify-around">
          {items.map(({ path, en, hi, icon: Icon }, index) => {
            ...
            return (
              <motion.button key={path} whileTap={{ scale: .88 }} whileHover={{ y: -2 }} onClick={() => nav(path)} className={`relative flex min-h-12 w-1/5 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-medium ${active ? "text-[#000080]" : "text-slate-400 hover:text-[#138808]"}`}>
                <motion.span animate={active ? { y: [0, -2, 0] } : { y: 0 }} transition={{ duration: 2.2, repeat: active ? Infinity : 0, delay: index * .08, ease: "easeInOut" }} className={`flex h-7 w-8 items-center justify-center rounded-lg ${active ? `bg-gradient-to-br ${navGradient[path]} shadow-sm` : ""}`}>
      ...
      {guest && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-[#000080]/20 p-4 backdrop-blur-sm sm:items-center">
          <motion.div initial={{ opacity: 0, y: 30, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="w-full max-w-sm rounded-3xl border border-orange-100 bg-white p-6 shadow-2xl">
            <div className="mb-5 flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-orange-100 to-green-100 text-[#000080]"><User className="h-5 w-5" /></div>
