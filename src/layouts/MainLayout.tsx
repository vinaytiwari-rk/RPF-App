...
export default function MainLayout() {
  ...
  if (isAdmin)
    return (
      <div
        className="min-h-screen w-full bg-white font-sans text-slate-900"
        style={tricolorPattern}
      >
  ...
  const nav = (p: string) => {
    if (
      user?.role === "guest" &&
      (p === "/services" || p === "/community" || p === "/notifications")
    ) {
      setGuest(true);
      return;
    }
  ...
  return (
    <div
      className="min-h-screen w-full overflow-hidden bg-white font-sans selection:bg-orange-100 selection:text-[#000080]"
      style={tricolorPattern}
    >
      <div
        className="fixed inset-x-0 top-0 z-[60] h-0.5 bg-gradient-to-r from-[#FF9933] via-white to-[#138808]"
        aria-hidden="true"
      />
          ...
          <div className="flex min-w-0 items-center gap-2.5">
            {!root ? (
              ...
              <motion.div
                ...
                className="relative flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl border border-orange-200 bg-white shadow-sm sm:h-10 sm:w-10"
              >
                <img
      ...
      {(location.pathname === "/" || location.pathname === "/services") && (
        <motion.button
          ...
          onClick={() => nav("/donations")}
          aria-label="Donate"
          className="fixed bottom-[calc(4.75rem+env(safe-area-inset-bottom))] right-3.5 z-40 flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-[#FF9933] to-[#138808] text-white shadow-lg"
        >
          ...
          {items.map(({ path, en, hi, icon: Icon }, index) => {
            ...
            return (
              <motion.button
                ...
                whileHover={{ y: -2 }}
                onClick={() => nav(path)}
                className={`relative flex min-h-12 w-1/5 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-medium ${active ? "text-[#000080]" : "text-slate-400 hover:text-[#138808]"}`}
              >
                <motion.span
                  ...
                  className={`flex h-7 w-8 items-center justify-center rounded-lg ${active ? `bg-gradient-to-br ${navGradient[path]} shadow-sm` : ""}`}
                >
                  <Icon
                ...
                {active && (
                  <motion.span
                    layoutId="active-nav"
                    className="absolute bottom-0 h-0.5 w-7 rounded-full bg-gradient-to-r from-[#FF9933] via-white to-[#138808]"
                  />
                )}
                {path === "/notifications" && unread > 0 && (
                  <motion.span
                    ...
                    className="absolute right-[calc(50%-0.8rem)] top-1 h-1.5 w-1.5 rounded-full bg-[#FF9933] ring-2 ring-white"
                  />
                )}
              </motion.button>
            );
      ...
      {guest && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-[#000080]/20 p-4 backdrop-blur-sm sm:items-center">
          <motion.div
            ...
            className="w-full max-w-sm rounded-3xl border border-orange-100 bg-white p-6 shadow-2xl"
          >
            ...
            <div className="mt-6 flex gap-2">
              <button
                ...
                className="flex-1 rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-700"
              >
                Close
              </button>
              <button
                onClick={() => setGuest(false)}
                className="flex-1 rounded-xl bg-[#000080] px-4 py-3 text-sm font-semibold text-white"
              >
                Sign in
              </button>
            </div>
