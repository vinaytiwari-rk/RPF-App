...
function findConstituenciesByDistrict(district: string, state?: string) {
  ...
  const clean = (value: string) => value
    .toLowerCase()
    .replace(/\b(district|m corp|municipal corporation|city)\b/g, '')
    .replace(/[^a-z]/g, '');
...
/**
 * Resolve constituencies only from the verified constituency dataset.
 * There are deliberately no hardcoded PIN mappings, district heuristics,
 * or MP-only mock fallbacks. When multiple constituencies match, the
 * caller receives the list and must let the user choose.
 */
export function resolveConstituency(_pincode: string, district: string, _areas: string[] = [], state?: string) {
  ...
  return {
    vidhan_sabha: '',
    vidhan_sabhas,
    sansad_kshetra: sansads.length === 1 ? sansads[0] : '',
  };
}
