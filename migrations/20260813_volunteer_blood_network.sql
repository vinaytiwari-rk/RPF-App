-- RP Foundation: Volunteer User ID + Blood Network migration
-- Safe to run more than once.

ALTER TABLE volunteers
  ADD COLUMN IF NOT EXISTS username VARCHAR(255);

CREATE UNIQUE INDEX IF NOT EXISTS uq_volunteers_username_lower
  ON volunteers (LOWER(username))
...
CREATE INDEX IF NOT EXISTS idx_blood_request_group_status
  ON blood_requests (blood_group, status);
CREATE INDEX IF NOT EXISTS idx_blood_acceptance_expiry
  ON blood_request_acceptances (expires_at);
