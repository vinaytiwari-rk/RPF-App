-- Phase 1: canonical administrator-managed service content model.
-- Idempotent migration; preserves existing values when legacy columns exist.
...
BEGIN
  SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=current_schema() AND table_name='service_content' AND column_name='content_en') INTO has_content_en;
  SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=current_schema() AND table_name='service_content' AND column_name='content_hi') INTO has_content_hi;
  SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=current_schema() AND table_name='service_content' AND column_name='action_label_en') INTO has_action_label_en;
  SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=current_schema() AND table_name='service_content' AND column_name='action_label_hi') INTO has_action_label_hi;
  ...
  IF has_content_en OR has_content_hi OR has_action_label_en OR has_action_label_hi THEN
    EXECUTE format($sql$
      ...
      SET content = COALESCE(content, '{}'::jsonb) || jsonb_build_object(
        'en', jsonb_build_object('body', %s, 'actionLabel', %s),
        'hi', jsonb_build_object('body', %s, 'actionLabel', %s)
      )
